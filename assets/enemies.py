...
class PackedIceEnemy(Enemy):
    DISPLAY_NAME="Packed Ice"; BASE_HP=75; BASE_SPEED=38; KILL_REWARD=100
    def __init__(self, wave=1):
        super().__init__(wave)
...
class ErrorEnemy(Enemy):
    ...
    def draw(self, surf, hovered=False, detected=False):
        bob=math.sin(self._bob*2)*3; cx,cy=int(self.x),int(self.y+bob)
        # Jittery visual (static layers now)
        sl=pygame.Surface((60,60),pygame.SRCALPHA)
        pygame.draw.circle(sl,(255,50,50,80),(30,30),self.radius); surf.blit(sl,(cx-30,cy-30))
        pygame.draw.circle(surf,(30,30,30),(cx,cy),self.radius)
        pygame.draw.circle(surf,(255,60,60),(cx-3,cy-3),5)
